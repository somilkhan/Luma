...
import { BookOpen, Star, Play, Plus, Info, Search, X } from "lucide-react";
...
export default function ReadPage() {
  ...
  const [searchResults, setSearchResults] = React.useState<MangaItem[] | null>(null);
  const [searching, setSearching] = React.useState(false);
  const [activeGenre, setActiveGenre] = React.useState("All");
  
  // Modal State
  const [selectedItem, setSelectedItem] = React.useState<MangaItem | null>(null);
  ...
  React.useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.error("Failed to fetch read data:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  React.useEffect(() => {
    ...
    const delayDebounce = setTimeout(async () => {
      setSearching(true);
      try {
        const res = await fetch(`/api/read?query=${encodeURIComponent(searchQuery)}`);
        if (res.ok) {
          const payload = await res.json();
  ...
  const handleOpenModal = (item: MangaItem) => {
  ...
  const genresList = ["All", "Action", "Adventure", "Fantasy", "Drama", "Psychological", "Mystery", "Thriller"];

  // Helper to filter items based on genre
  const filterByGenre = (items: MangaItem[]) => {
  ...
  // Hero feature item
  const heroItem: MangaItem | null = data?.trending?.[0] || null;
    ...
    <div className="space-y-xl pb-xxl">
      ...
      {loading ? (
        ...
        <div className="relative w-full h-[320px] sm:h-[400px] md:h-[480px] lg:h-[520px] rounded-3xl overflow-hidden group select-none border border-glass-border bg-gradient-to-br from-zinc-900 via-zinc-950 to-black">
          ...
          <div className="absolute inset-0 z-0 bg-background">
            {heroItem.backdropPath ? (
              <Image
                ...
                priority
                sizes="100vw"
                className="object-cover object-top blur-[4px] opacity-40 transition-transform duration-[10000ms] ease-out scale-100 group-hover:scale-105"
          ...
          {/* Hero Meta & CTA Content, with mini poster block */}
          <div className="absolute inset-0 p-4 sm:p-lg md:p-xl z-10 flex flex-col md:flex-row items-end md:items-center gap-md sm:gap-xl select-none text-left justify-end md:justify-start">
            {/* Poster left block on desktop */}
            {heroItem.posterPath && (
              <div className="hidden md:block w-40 lg:w-52 aspect-[2/3] relative rounded-xl overflow-hidden border border-glass-border shadow-soft-lg flex-shrink-0">
                <Image
            ...
            <div className="flex flex-col items-start space-y-1.5 sm:space-y-sm md:space-y-md max-w-2xl w-full">
              <div className="flex items-center gap-xs flex-wrap">
                ...
                {heroItem.releaseYear && (
                  <span className="text-[9px] sm:text-[11px] text-secondary-text px-1.5 py-[2px] sm:px-sm sm:py-xs rounded-small bg-white/5">
                    {heroItem.releaseYear}
                  </span>
                )}
                {heroItem.author && (
                  <span className="text-[9px] sm:text-[11px] text-muted-text px-1.5 py-[2px] sm:px-sm sm:py-xs rounded-small">
                    By {heroItem.author}
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-xs sm:gap-sm pt-xs sm:pt-sm">
                <Button variant="primary" size="sm" className="sm:size-md md:size-lg gap-xs sm:gap-sm font-semibold text-xs sm:text-sm px-2.5 py-1.5 sm:px-md sm:py-sm" onClick={() => handleOpenModal(heroItem)}>
                  <BookOpen className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5" /> Start Reading
                </Button>
      ...
      <div className="space-y-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-md">
          <div>
            <h2 className="text-heading font-bold text-primary-text">Literary Library</h2>
            <p className="text-small text-muted-text">Immerse yourself in legendary Manga, high-fidelity Manhwa, and novels</p>
          </div>
      ...
      <div className="space-y-xl">
        {loading ? (
          ...
          <div className="space-y-md">
            ...
            {searching ? (
              ...
              <div className="flex flex-col items-center justify-center py-xxl text-center">
                <BookOpen className="w-12 h-12 text-muted-text stroke-1 mb-md" />
                <p className="text-body text-secondary-text font-medium">No results found</p>
                <p className="text-small text-muted-text mt-xs">Try searching for other top-tier volumes.</p>
              </div>
            )}
          </div>
        ) : (
          ...
          <div className="space-y-xl">
            {/* Section 1: Trending Releases (Landscape Cards as top featured row) */}
            {data?.trending && data.trending.length > 0 && (
              <div className="space-y-sm text-left">
                <div className="border-l-[3.5px] border-red-600 pl-3">
                  <h3 className="text-subheading font-bold text-primary-text leading-none">Trending Releases</h3>
                </div>
            ...
            {/* Section 2: All-Time Classics (Portrait Cards) */}
            {data?.popular && data.popular.length > 0 && (
              <div className="space-y-sm text-left">
                <div className="border-l-[3.5px] border-red-600 pl-3">
                  <h3 className="text-subheading font-bold text-primary-text leading-none">All-Time Classics</h3>
                </div>
                <div className="flex gap-md overflow-x-auto pb-md scrollbar-none snap-x snap-mandatory px-xs -mx-xs pt-1">
            ...
            {/* Vertical Genre Rows */}
            {["Action", "Adventure", "Fantasy", "Drama", "Psychological", "Mystery", "Thriller"].map((genreName) => {
          ...
          <div className="space-y-lg text-left">
            ...
            <div className="relative w-full h-56 rounded-xl overflow-hidden border border-glass-border bg-secondary-surface">
              {selectedItem.backdropPath ? (
                ...
                <Image
                  ...
                  alt={selectedItem.title}
                  fill
                  className="object-cover object-center opacity-60 blur-[1px]"
                  referrerPolicy="no-referrer"
                />
              ) : null}
              <div className="absolute inset-0 bg-gradient-to-t from-primary-surface via-transparent to-black/40" />
              
              <div className="absolute inset-0 flex items-center gap-md px-md z-10 mt-xl">
                {selectedItem.posterPath && (
                  <div className="relative h-24 aspect-[2/3] rounded border border-glass-border shadow-md overflow-hidden shrink-0">
                    <Image
                      src={selectedItem.posterPath}
                      alt={selectedItem.title}
                      fill
                ...
                <div>
                  ...
                  <h2 className="text-subheading md:text-heading font-bold text-primary-text drop-shadow-md mt-sm leading-tight">{selectedItem.title}</h2>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-md text-small text-secondary-text font-medium">
              {selectedItem.releaseYear && (
                <span className="bg-secondary-surface border border-glass-border px-sm py-[2px] rounded-small">
                  Released {selectedItem.releaseYear}
                </span>
              )}
              {selectedItem.rating && (
            ...
            <div className="space-y-sm">
              ...
              <p className="text-small text-secondary-text leading-relaxed">
                {selectedItem.overview || "No synopsis available for this volume."}
              </p>
            </div>
