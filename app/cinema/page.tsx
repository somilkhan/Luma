...
import { Film, Star, Play, Plus, Info, Search, X } from "lucide-react";
...
export default function CinemaPage() {
  ...
  const [searchResults, setSearchResults] = React.useState<CinemaItem[] | null>(null);
  const [searching, setSearching] = React.useState(false);
  const [activeGenre, setActiveGenre] = React.useState("All");
  
  // Modal State
  const [selectedItem, setSelectedItem] = React.useState<CinemaItem | null>(null);
  ...
  React.useEffect(() => {
    ...
    const delayDebounce = setTimeout(async () => {
      setSearching(true);
      try {
        const res = await fetch(`/api/cinema?query=${encodeURIComponent(searchQuery)}`);
        if (res.ok) {
          const payload = await res.json();
  ...
  const handleOpenModal = (item: CinemaItem) => {
  ...
  const genresList = ["All", "Sci-Fi", "Drama", "Action", "Adventure", "Crime", "Mystery"];

  // Helper to filter items based on genre
  const filterByGenre = (items: CinemaItem[]) => {
  ...
  // Hero feature item: Dune: Part Two or first item from trending
  const heroItem: CinemaItem | null = data?.trending?.[0] || null;
    ...
    <div className="space-y-xl pb-xxl">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-lg md:p-xl z-10 flex flex-col items-start space-y-1.5 sm:space-y-sm md:space-y-md max-w-2xl w-full">
            ...
            <div className="flex items-center gap-xs sm:gap-sm pt-xs sm:pt-sm">
              <Button variant="primary" size="sm" className="sm:size-md md:size-lg gap-xs sm:gap-sm font-semibold text-xs sm:text-sm px-2.5 py-1.5 sm:px-md sm:py-sm" onClick={() => handleOpenModal(heroItem)}>
                <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 fill-current" /> Play Trailer
              </Button>
              <Button variant="glass" size="sm" className="sm:size-md md:size-lg gap-xs sm:gap-sm text-xs sm:text-sm px-2.5 py-1.5 sm:px-md sm:py-sm" onClick={() => handleOpenModal(heroItem)}>
                <Info className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5" /> More Info
              </Button>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-md">
          <div>
            <h2 className="text-heading font-bold text-primary-text">Cinema Catalogue</h2>
            <p className="text-small text-muted-text">Discover industry-leading blockbuster movies and premium series</p>
          </div>
      ...
      <div className="space-y-xl">
        {loading ? (
          ...
          <div className="space-y-md">
            ...
            {searching ? (
              ...
              <div className="flex flex-col items-center justify-center py-xxl text-center">
                <Film className="w-12 h-12 text-muted-text stroke-1 mb-md" />
                <p className="text-body text-secondary-text font-medium">No titles found</p>
                <p className="text-small text-muted-text mt-xs">Try searching for other blockbusters or shows.</p>
              </div>
            )}
          </div>
        ) : (
          ...
          <div className="space-y-xl">
            {/* Section 1: Trending Now (Landscape Cards as top featured row) */}
            {data?.trending && data.trending.length > 0 && (
              <div className="space-y-sm text-left">
                <div className="border-l-[3.5px] border-red-600 pl-3">
                  <h3 className="text-subheading font-bold text-primary-text leading-none">Trending Now</h3>
                </div>
            ...
            {/* Section 2: Popular Blockbusters (Portrait Cards) */}
            {data?.popular && data.popular.length > 0 && (
              <div className="space-y-sm text-left">
                <div className="border-l-[3.5px] border-red-600 pl-3">
                  <h3 className="text-subheading font-bold text-primary-text leading-none">Popular Blockbusters</h3>
                </div>
                <div className="flex gap-md overflow-x-auto pb-md scrollbar-none snap-x snap-mandatory px-xs -mx-xs pt-1">
            ...
            {/* Section 3: Critically Acclaimed Series (Portrait Cards) */}
            {data?.topRated && data.topRated.length > 0 && (
              <div className="space-y-sm text-left">
                <div className="border-l-[3.5px] border-red-600 pl-3">
                  <h3 className="text-subheading font-bold text-primary-text leading-none">Critically Acclaimed Series</h3>
                </div>
                <div className="flex gap-md overflow-x-auto pb-md scrollbar-none snap-x snap-mandatory px-xs -mx-xs pt-1">
            ...
            {/* Vertical Genre Rows */}
            {["Sci-Fi", "Drama", "Action", "Adventure", "Crime", "Mystery"].map((genreName) => {
            ...
            {/* Section 4: Highly Anticipated (Portrait Cards) */}
            {data?.upcoming && data.upcoming.length > 0 && (
              <div className="space-y-sm text-left">
                <div className="border-l-[3.5px] border-red-600 pl-3">
                  <h3 className="text-subheading font-bold text-primary-text leading-none">Highly Anticipated</h3>
                </div>
                <div className="flex gap-md overflow-x-auto pb-md scrollbar-none snap-x snap-mandatory px-xs -mx-xs pt-1">
